...
// New functions for Questions 3-8
static void runNumNodes(struct node **t, int argc, char *argv[]);
static void runCountOdds(struct node **t, int argc, char *argv[]);
static void runCountInternal(struct node **t, int argc, char *argv[]);
static void runHeight(struct node **t, int argc, char *argv[]);
static void runNodeLevel(struct node **t, int argc, char *argv[]);
static void runCountGreater(struct node **t, int argc, char *argv[]);
...
static Command COMMANDS[] = {
	...
	// New commands for Questions 3-8
	{"n", runNumNodes,     "",          "count the total number of nodes"},
	{"o", runCountOdds,    "",          "count the number of odd values"},
	{"i", runCountInternal, "",         "count the number of internal nodes"},
	{"h", runHeight,       "",          "compute the height of the tree"},
	{"v", runNodeLevel,    "<num>",     "find the level of a node with a given key"},
	{"g", runCountGreater, "<num>",     "count the number of values greater than a given value"},
...
static void runCountOdds(struct node **t, int argc, char *argv[]) {
	...
	int count = bstCountOdds(*t);
	printf("The BST contains %d odd %s\n", count,
...
static void runCountInternal(struct node **t, int argc, char *argv[]) {
	...
	int count = bstCountInternal(*t);
	printf("The BST contains %d internal %s\n", count,
	       count == 1 ? "node" : "nodes");
}
...
static void runNodeLevel(struct node **t, int argc, char *argv[]) {
	...
	if (level == -1) {
		...
		printf("The value %d is at level %d in the BST\n", key, level);
	}
}
...
static void runCountGreater(struct node **t, int argc, char *argv[]) {
	...
	int val = 0;
	char c;
	if (sscanf(argv[1], "%d%c", &val, &c) != 1) {
		printf("Error: Invalid value '%s'\n", argv[1]);
		return;
	}

	int count = bstCountGreater(*t, val);
	printf("The BST contains %d %s greater than %d\n", count, count == 1 ? 
            "value" : "values", val);
}
