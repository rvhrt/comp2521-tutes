...
int bstNumNodes(struct node *t) {
    ...
    // Add one to our total count, counting the number of nodes in each 
    // subtree, and adding these two together as well.
...
int bstCountOdds(struct node *t) {
    ...
    // Add one to our total count IF the current node contains an odd value.
    int addVal = 0;
    if (abs(t->value) % 2 == 1) {
        addVal = 1;
    }

    // Recursively add the total number of odd nodes in each subtree,
    // adding this all together.
    return addVal + bstCountOdds(t->left) + bstCountOdds(t->right);
}
...
int bstCountInternal(struct node *t) {
    ...
    // Add one to our total count IF the current node has at least one child 
    // node.
    int addVal = 0;
    ...
    // Recursively calculate the total number of internal nodes in each subtree,
    // adding this all together.
    return addVal + bstCountInternal(t->left) + bstCountInternal(t->right);
}
...
int bstHeight(struct node *t) {
    ...
    return max(1 + bstHeight(t->left), 1 + bstHeight(t->right));
}

// Simple helper function to return max of two numbers.
static int max(int a, int b) {
    if (a > b) {
        return a;
    } 
...
// Returns the level of the node containing the given key, or -1 if the key is 
// not in the BST
int bstNodeLevel(struct node *t, int key) {
    // We will do this iteratively. (but it can be done recursively!)
    struct node *curr = t;
    int notFound = -1;
    int currLevel = 0;
    ...
    // Go through the tree:
    while (curr != NULL) {
        // If we are on the key, return its level,
        // If we are on a value greater than the key, go left,
        // If we are on a value smaller than the key, go right.
        if (curr->value == key) {
            return currLevel;
    ...
    // If we have not returned by now, the key does not exist. 
    return notFound;
}
...
int bstCountGreater(struct node *t, int val){
    ...
    // If the current node we are on is greater, add 1 to the count, and search
    // subtrees.
    if (t->value > val) {
        return 1 + bstCountGreater(t->left, val) + bstCountGreater(t->right, val);
    } 

    // Otherwise (that is, we are on a node that is less than or equal to the 
    // value), we ONLY search through its right subtree. This is because we know
    // that all the values in the left subtree are guaranteed to be smaller
    // than the current value (this is an inherent property of BSTs), so we can 
    // safely ignore them.
    // Note that we do not add 1 here as we are on a node that does not 
    // match our criteria (i.e. greater than val).
